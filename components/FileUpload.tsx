...
export const FileUpload: React.FC<FileUploadProps> = ({ onFilesChange, files }) => {
  ...
  /**
   * Optimized Image Preprocessing for Structural Discovery.
   * Sharpens edges of tables and charts to help Gemini Vision identify grid lines and data points.
   */
  const preprocessCanvas = (canvas: HTMLCanvasElement) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      // Strict Whitening for document background
    ...
    // 2. High-Pass Sharpening (Edge Enhancement for Tables)
    const kernel = [
       -1, -1, -1,
       -1,  9, -1,
       -1, -1, -1
  ...
  const extractTextFromPdf = async (arrayBuffer: ArrayBuffer, fileName: string): Promise<string> => {
    ...
    // Fallback to Cognitive OCR if text layer is empty or document is likely complex (e.g. scanned tables)
    if (fullText.trim().length < 100 * pdf.numPages && pdf.numPages > 0) {
      ...
      setIsCognitiveOcr(false);
       ...
       // Optional: Still run deep structural analysis if the file name contains "table" or "report"
       // to ensure charts and grids are captured even if text exists.
    }
  ...
  const performCognitiveOcr = async (pdf: any): Promise<string> => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      combinedText += `--- PAGE ${i} (STRUCTURAL EXTRACTION) ---\n${extractedText}\n\n`;
    }
    setOcrProgress(0);
    return combinedText;
  };
  ...
  return (
    <div className="space-y-4">
      <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-indigo-400 cursor-pointer bg-white/50 transition-all" onClick={() => fileInputRef.current?.click()}>
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-slate-400 text-sm mt-1">Multi-Modal Structural Parsing for Complex Layouts</p>
        </div>
      </div>
            ...
            {file.status === 'processing' && (
              <div className="flex flex-col gap-1">
                <span className="text-[9px] font-black text-indigo-600 uppercase tracking-widest animate-pulse">
                  {isCognitiveOcr ? `Deep Structural Scan (${ocrProgress}%)` : 'Analyzing Layout...'}
                </span>
                <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 animate-[progress_1.5s_infinite] w-full origin-left"></div></div>
              </div>
            )}
            {file.status === 'ready' && <span className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest flex items-center gap-1"><ICONS.Shield className="w-2 h-2" /> Structure Parsed</span>}
          </div>
        ))}
      </div>
